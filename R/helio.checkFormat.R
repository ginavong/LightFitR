...
helio.checkFormat = function(check_matrix){ # Check that the regime matrix is formatted correctly
  ...
  ledMessage = paste('Please check that your rows correspond with all ', nrow(helio.dyna.leds), ' LEDs in the heliospectra DYNA: ', paste(LightFitR::helio.dyna.leds$name, collapse=', '), sep='')
